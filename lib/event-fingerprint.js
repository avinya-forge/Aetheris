...
function fingerprintEvent(event) {
  ...
  let raw = event.hash || event.id || event.url;

  if (!raw && typeof event.text === 'string') {
    // Normalize text: remove punctuation, extra whitespace, lowercase
    const normalizedText = event.text
      .replace(/[^\w\s]/g, '')
      .replace(/\s+/g, ' ')
      .trim()
      .toLowerCase();
    raw = normalizedText ? normalizedText.slice(0, 120) : null;
  }
...
export { fingerprintEvent, isNewEvent, markEventSeen, KV_PREFIX };
