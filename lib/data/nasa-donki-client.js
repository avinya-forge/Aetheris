...
const { mapDonkiEvent, DONKI_IMPACT_BY_TYPE } = require('./space-weather-mapper.js');
...
async function fetchEventType(type, apiKey, startDate, fetcher) {
  ...
  return data.map(event => mapDonkiEvent(event, type));
}

/**
 * Fetch all NASA DONKI event types for the past 24 hours.
 */
async function fetchNasaDonki(apiKey = 'DEMO_KEY', fetcher = globalThis.fetch, now = Date.now()) {
