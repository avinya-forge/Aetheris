 ...
 *
 * Standard 4: AI synthesis must be derived from 20+ validated sources.
 *
...
async function synthesizeSources(sources, synthesizer = null) {
  ...
  // Attempt AI synthesis when a synthesizer is injected and source count >= 20
  if (synthesizer && sources.length >= 20) {
    try {
      const aiResult = await synthesizer(combinedText);
      if (aiResult && typeof aiResult === 'string' && aiResult.trim()) {
        const aiWordCount = aiResult.trim().split(/\s+/).length;
        if (aiWordCount > 30) {
          throw new Error('AI synthesis exceeds 30 words');
        }
        return aiResult.trim();
      }
    } catch (err) {
      if (err.message === 'AI synthesis exceeds 30 words') {
        throw err;
      }
