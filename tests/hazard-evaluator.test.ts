...
assert.strictEqual(
  ...
  'evaluateHazard: cold warning for -10°C'
);
...
assert.strictEqual(
  ...
  'evaluateHazard: storm warning for 100km/h'
);

// Test safe conditions
assert.strictEqual(evaluateHazard({ temperature: 25, windSpeed: 20 }), null, 'evaluateHazard: safe conditions');
assert.strictEqual(evaluateHazard({}), null, 'evaluateHazard: empty input');
