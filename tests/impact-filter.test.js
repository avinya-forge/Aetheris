...
assert.strictEqual(result1.length, 3, 'Must match expected value');
assert.strictEqual(result1[0].id, 1, 'Must match expected value');
assert.strictEqual(result1[1].id, 3, 'Must match expected value');
assert.strictEqual(result1[2].id, 5, 'Must match expected value');

// Test 2: Impact score + county
const threshold2 = { minImpactScore: 5, county: 'King' };
const result2 = filterByImpact(newsData, threshold2);
assert.strictEqual(result2.length, 1, 'Must match expected value');
assert.strictEqual(result2[0].id, 1, 'Must match expected value'); // Item 5 has no county, now it's excluded because `item.county` is undefined which !== 'King'
...
assert.strictEqual(result3.length, 1, 'Must match expected value');
assert.strictEqual(result3[0].id, 2, 'Must match expected value');
