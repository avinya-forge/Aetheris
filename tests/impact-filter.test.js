...
assert.strictEqual(result1[1].id, 3);
assert.strictEqual(result1[2].id, 5);
...
assert.strictEqual(result2[0].id, 1); // Item 5 has no county, now it's excluded because `item.county` is undefined which !== 'King'
...
assert.strictEqual(result3[0].id, 2);
...
assert.strictEqual(result4[0].id, 1);
