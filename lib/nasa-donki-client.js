...
const { DONKI_IMPACT_BY_TYPE } = require('./space-weather-mapper');
 ...
 */
async function fetchNasaDonki(apiKey = 'DEMO_KEY', fetcher = globalThis.fetch, now) {
  if (!now) { throw new Error('now timestamp must be injected'); }
  const startDate = new Date(now - 86_400_000).toISOString().slice(0, 10);
  const results = {};
...
export { fetchNasaDonki, fetchEventType, buildUrl, EVENT_TYPES };
